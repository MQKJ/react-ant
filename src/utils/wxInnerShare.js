...
function shareAction (isHide) {
  if (isHide) {
    ...
    let title = '80万出行保障等你来抢';
    let pathname = window.location.pathname.substr(window.location.pathname.length - 11); // 获取进入页面的路径
    ...
    let RedParam = JSON.parse(sessionStorage.getItem('AgentForParam')); // 来自购买成功页的缓存
    ...
    let imgUrl = `${origin}/innercenter/CXBA/static/img/wxshare.png`;
    let link = `${origin}/innercenter/cxbOrderForward/validateActivity?${param}`;
    wx.ready(function () {
      wx.hideMenuItems({
...
export default function getSign (api, flag, isShare) {
    axios.post('/api1/innercenter/wechatsign/signature?url=' + encodeURIComponent(window.location.href)).then(res => {
      if (res && res.data) {
          let data = res.data;
        wx.config({
          debug: true,
